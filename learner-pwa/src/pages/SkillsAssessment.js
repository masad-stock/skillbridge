...
import offlineAssessmentEngine from '../services/offlineStorage/OfflineAssessmentEngine';
...
function SkillsAssessment() {
    ...
    const [useOfflineEngine, setUseOfflineEngine] = useState(false);
    ...
    const initializeAssessment = useCallback(async () => {
        // Try to use offline engine first (works both online and offline)
        try {
            const assessment = await offlineAssessmentEngine.loadAssessment();
            setQuestions(assessment.questions);
            setUseOfflineEngine(true);
            console.log('[SkillsAssessment] Using offline assessment engine');
        } catch (error) {
            console.error('[SkillsAssessment] Failed to load offline assessment:', error);
            // Fallback to online assessment
            await aiAssessment.initializeModel();
            setQuestions(getAssessmentQuestions());
            setUseOfflineEngine(false);
            console.log('[SkillsAssessment] Using online assessment engine');
        }
    }, []);
    ...
    const handleAnswer = (selectedOption) => {
        ...
        setTimeout(async () => {
            ...
            // Submit answer to offline engine if using it
            if (useOfflineEngine) {
                try {
                    await offlineAssessmentEngine.submitAnswer(
                        questions[currentQuestion].id,
                        selectedOption
                    );
                } catch (error) {
                    console.error('[SkillsAssessment] Failed to submit answer offline:', error);
                }
            }
    ...
    const completeAssessment = async (allResponses) => {
        ...
        if (useOfflineEngine) {
            // Use offline assessment engine
            try {
                const offlineResults = await offlineAssessmentEngine.calculateResults();
                const learningPath = await offlineAssessmentEngine.generateLearningPath(offlineResults);

                const totalTime = new Date() - timeStarted;
                assessmentResults = {
                    skillsProfile: offlineResults.skillLevels,
                    learningPath,
                    completedAt: offlineResults.completedAt,
                    totalScore: offlineResults.overallScore,
                    maxScore: 100,
                    timeSpent: Math.round(totalTime / 1000 / 60), // minutes
                    responses: allResponses.length,
                    syncStatus: offlineResults.syncStatus,
                    isOffline: isOffline
                };

                console.log('[SkillsAssessment] Assessment completed offline');
            } catch (error) {
                console.error('[SkillsAssessment] Failed to calculate offline results:', error);
                // Fallback to online calculation
                assessmentResults = await calculateOnlineResults(allResponses);
            }
        } else {
            // Use online assessment engine
            assessmentResults = await calculateOnlineResults(allResponses);
        }
        ...
        dispatch({ type: 'SET_SKILLS_PROFILE', payload: assessmentResults.skillsProfile });
        dispatch({ type: 'SET_LEARNING_PATH', payload: assessmentResults.learningPath });
        dispatch({ type: 'SET_ASSESSMENT_RESULTS', payload: assessmentResults });
    };

    const calculateOnlineResults = async (allResponses) => {
    ...
    if (isComplete && results) {
        const overallScore = results.totalScore;
                            ...
                            <Card.Body className="p-4">
                                ...
                                {results.isOffline && (
                                    <Alert variant="info" className="mb-4">
                                        ...
                                        <p className="mb-0">
                                            Your assessment was completed offline and results were calculated locally.
                                            Your results will be automatically synced to the server when you're back online.
                                        </p>
                                    </Alert>
                                )}
                                ...
                                <div className="text-center mt-4">
                                    <Button
                                        onClick={() => navigate('/learning', { state: { autoStartFirst: true } })}
                                        variant="primary"
                                        size="lg"
                                ...
                                <small className="text-muted">
                                    ...
                                    {isOffline && (
                                        <span className="d-block mt-2">
                                            <span className="me-1">📴</span>
                                            Working offline - your results will sync automatically when you're back online.
                                        </span>
                                    )}
                                </small>
                            </div>
